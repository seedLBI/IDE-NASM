...
void AuthorPopup::Draw() {
	...
	ImGui::OpenPopup("###POPUP_ABOUT_PROGRAMM");
	...
	float height_window = ImGui::GetTextLineHeight() * 20.f;
	...
	if (ImGui::BeginPopupModal((tr("authorPopup.title") + "###POPUP_ABOUT_PROGRAMM").c_str(), &isOpen, flagsWindow)) {
		...
		ImGui::TextWrapped(tr("authorPopup.desc").c_str());
		ImGui::Text("");

		TextCenteredOnLine(tr("authorPopup.workers").c_str(),0,0);
		ImGui::Text("");
		aa = ImGui::GetCursorPos();
		aa.x += 17;
		ImGui::SetCursorPos(aa);
		ImGui::Text(tr("authorPopup.progger").c_str());
		aa = ImGui::GetCursorPos();
		aa.x += 17;
		ImGui::SetCursorPos(aa);
		ImGui::Text(tr("authorPopup.scietificSupervisor").c_str());
		ImGui::Text("");

		TextCenteredOnLine(tr("authorPopup.myContacts").c_str(), 0, 0);
		...
		ImGui::Text("Github: ");
		ImGui::SameLine();
		if (ImGui::Button("github.com/seedLBI/IDE-NASM")) {
			ShellExecute(0, 0, L"https://github.com/seedLBI/IDE-NASM", 0, 0, SW_SHOW);
		}
		aa = ImGui::GetCursorPos();
		aa.x += 17;
		ImGui::SetCursorPos(aa);
