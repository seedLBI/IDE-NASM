...
NotificationManager::NotificationManager(Window* MainWindow, FPS_Timer* fps_limiter) : ISettingObject(u8"Уведомления",u8"Общие") {
...
void NotificationManager::Update() {
	...
	glfwGetWindowSize(MainWindow->GetHandle(), &size_x, &size_y);
	...
	for (int i = 0; i < notifs.size(); i++) {
		...
		notifs[i].Update(static_cast<float>(fps_limiter->GetDeltaTime()));
