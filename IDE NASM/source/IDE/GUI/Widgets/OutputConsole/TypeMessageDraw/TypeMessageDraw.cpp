...
void TypeMessageDraw::Draw(const std::vector<TypeMessageConsole>& types) {
	...
	if (types.size() == 1) {
		...
		const std::string& translation = Data[str_type]["translation"][gl()].get<std::string>();
		const std::string& description = Data[str_type]["description"][gl()].get<std::string>();
		...
		if (ImGui::IsItemHovered()) {
			...
			ImGui::TextColored(color_bold_text, std::string(tr("typeMessageDraw.type.tooltip") + " - " + translation).c_str());
			ImGui::PopFont();
