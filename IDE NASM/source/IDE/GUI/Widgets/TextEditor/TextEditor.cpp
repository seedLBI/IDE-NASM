...
TextEditor::TextEditor(FPS_Timer* fps_limiter, const std::wstring& Path):
	  mLineSpacing(1.0f)
	, mUndoIndex(0)
	, mTabSize(4)
	, mOverwrite(false)
	...
	, mStartTime(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count())
{
	...
	SetLanguageDefinition(LanguageDefinition::NASM());
...
#ifdef _DEBUG
	std::cout << "ENTER -> TextEditor::~TextEditor()\n";
#endif

#ifdef _DEBUG
	std::cout << "LEAVE -> TextEditor::~TextEditor()\n";
#endif
}

void TextEditor::ChangePath(const std::wstring& Path) {
	PathFile = Path;
}
...
std::wstring TextEditor::GetPath() {
	return PathFile;
}
...
std::string TextEditor::GetNameChild() {
	return ChildName;
}
bool TextEditor::GetIsFocused() {
	return IsFocused;
}
...
void TextEditor::SetLanguageDefinition(const LanguageDefinition & aLanguageDef)
{
	...
	for (auto& r : mLanguageDefinition.mTokenRegexStrings) {
		...
		bool haveNOI = false;
		if (r.first.substr(0,3) == "NOI")
		{
			haveNOI = true;
			r.first.erase(r.first.begin(), r.first.begin() + 3);
		}
		if (haveNOI)
			mRegexList.push_back(std::make_pair(std::regex(r.first), r.second));
		else
...
ImU32 TextEditor::GetGlyphColor(const Glyph & aGlyph) const
{
	...
	if (aGlyph.mColorCustomIndex != -1) {
		return ImColor(HighlighterSyntax::Instance().GetColorFromType(HighlighterSyntaxType(aGlyph.mColorCustomIndex)));
...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		if (!IsReadOnly() && !mWindowFind && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Z)) {
			mErrorMarkers.clear();
			Undo();
		}
		...
		else if (ctrl && !alt && ImGui::IsKeyPressed(ImGuiKey_F)) {
			Toggle_OpenFindWindow();
		}
		else if (!IsReadOnly() && !mWindowFind && !ctrl && !shift && alt && ImGui::IsKeyPressed(ImGuiKey_Backspace)) {
			mErrorMarkers.clear();
			Undo();
		}
		else if (!IsReadOnly() && !mWindowFind && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Y)) {
			mErrorMarkers.clear();
			Redo();
		}
		else if (!ctrl && !mWindowFind && !alt && ImGui::IsKeyPressed(ImGuiKey_UpArrow)) {
			mErrorMarkers.clear();
			MoveUp(1, shift);
		}
		else if (!ctrl && !mWindowFind && !alt && ImGui::IsKeyPressed(ImGuiKey_DownArrow)) {
			mErrorMarkers.clear();
			MoveDown(1, shift);
		}
		else if (!alt && !mWindowFind && ImGui::IsKeyPressed(ImGuiKey_LeftArrow)) {
			mErrorMarkers.clear();
			MoveLeft(1, shift, ctrl);
		}
		else if (!alt && !mWindowFind && ImGui::IsKeyPressed(ImGuiKey_RightArrow)) {
			mErrorMarkers.clear();
			MoveRight(1, shift, ctrl);
		}
		else if (!alt && !mWindowFind && ImGui::IsKeyPressed((ImGuiKey_PageUp))) {
			mErrorMarkers.clear();
			MoveUp(GetPageSize() - 4, shift);
		}
		else if (!alt && !mWindowFind && ImGui::IsKeyPressed((ImGuiKey_PageDown))) {
			mErrorMarkers.clear();
			MoveDown(GetPageSize() - 4, shift);
		}
		else if (!alt && !mWindowFind && ctrl && ImGui::IsKeyPressed((ImGuiKey_Home))) {
			mErrorMarkers.clear();
			MoveTop(shift);
		}
		else if (ctrl && !mWindowFind && !alt && ImGui::IsKeyPressed((ImGuiKey_End))) {
			mErrorMarkers.clear();
			MoveBottom(shift);
		}
		else if (!ctrl && !mWindowFind && !alt && ImGui::IsKeyPressed((ImGuiKey_Home))) {
			mErrorMarkers.clear();
			MoveHome(shift);
		}
		else if (!ctrl && !mWindowFind && !alt && ImGui::IsKeyPressed((ImGuiKey_End))) {
		...
		else if (!IsReadOnly() && !mWindowFind && !ctrl && !shift && !alt && ImGui::IsKeyPressed((ImGuiKey_Delete))) {
			mErrorMarkers.clear();
			Delete();
		}
		else if (!IsReadOnly() && !mWindowFind && !ctrl && !shift && !alt && ImGui::IsKeyPressed((ImGuiKey_Backspace))) {
		...
		else if (!ctrl && !mWindowFind && !shift && !alt && ImGui::IsKeyPressed((ImGuiKey_Insert))) {
			mErrorMarkers.clear();
			mOverwrite ^= true;
		}
		else if (ctrl && !mWindowFind && !shift && !alt && ImGui::IsKeyPressed((ImGuiKey_Insert))) {
		...
		else if (ctrl && !mWindowFind && !shift && !alt && ImGui::IsKeyPressed((ImGuiKey_C))) {
			mErrorMarkers.clear();
			Copy();
		}
		else if (!IsReadOnly() && !mWindowFind && !ctrl && shift && !alt && ImGui::IsKeyPressed((ImGuiKey_Insert))) {
			mErrorMarkers.clear();
			Paste();
		}
		else if (!IsReadOnly() && !mWindowFind && ctrl && !shift && !alt && ImGui::IsKeyPressed((ImGuiKey_V))) {
			mErrorMarkers.clear();
			Paste();
		}
		else if (ctrl && !mWindowFind && !shift && !alt && ImGui::IsKeyPressed((ImGuiKey_X))) {
			mErrorMarkers.clear();
			Cut();
		}
		else if (!ctrl && !mWindowFind && shift && !alt && ImGui::IsKeyPressed((ImGuiKey_Delete))) {
			mErrorMarkers.clear();
			Cut();
		}
		else if (ctrl && !mWindowFind && !shift && !alt && ImGui::IsKeyPressed((ImGuiKey_A))) {
			mErrorMarkers.clear();
			SelectAll();
		}
		else if (!IsReadOnly() && !mWindowFind && !ctrl && !shift && !alt && ImGui::IsKeyPressed((ImGuiKey_Enter))) {
			mErrorMarkers.clear();
			EnterCharacter('\n', false);
		}
		else if (!IsReadOnly() && !mWindowFind && !ctrl && !alt && ImGui::IsKeyPressed((ImGuiKey_Tab))) {
			mErrorMarkers.clear();
			EnterCharacter('\t', shift);
		}
		else if (!IsReadOnly() && alt && !ctrl && !shift && !mWindowFind && ImGui::IsKeyPressed((ImGuiKey_UpArrow))) {
			MoveLineUp(false);
		}
		else if (!IsReadOnly() && alt && !ctrl && !shift && !mWindowFind && ImGui::IsKeyPressed((ImGuiKey_DownArrow))) {
			MoveLineDown(false);
		}
		...
		if (ImGui::IsMouseDragging(0) && ImGui::IsMouseDown(0) && IsDragSelection)
		{
			...
			if (Delta_Vertical_Down > 0) {
				...
				if (NeedChangeSymbol >= 0.2f) {
					int countAdd = static_cast<int>(NeedChangeSymbol / 0.2f);
					if (countAdd > 3)
						countAdd = 3;
				...
				if (NeedChangeSymbol >= 0.2f) {
					int countSub = static_cast<int>(NeedChangeSymbol / 0.2f);
					if (countSub > 3)
						countSub = 3;
...
void TextEditor::HandleMouseInputs_Step2() {
	...
	if (ImGui::IsWindowHovered())
	{
		...
		if (ctrl)
		{
			...
			mVirtualFontSize += io.MouseWheel * 0.125f;
...
void TextEditor::Render()
{
	...
	if (!mLines.empty())
	{
			...
			mousePos.y >= cursorScreenPos.y && mousePos.y <= cursorScreenPos.y + contentSize.y)
		{
			...
			std::string id = GetWordAt(mouseCoord);


			if (!id.empty()) {


				std::string id_lowered = id;
				for (int i = 0; i < id_lowered.size(); i++)
					id_lowered[i] = tolower(id_lowered[i]);


				if (id_lowered.empty() == false && id_lowered.front() == ' ')
					id_lowered.erase(id_lowered.begin());
				if (id_lowered.empty() == false && id_lowered.back() == ' ')
					id_lowered.pop_back();

				if (id.empty() == false && id.front() == ' ')
					id.erase(id.begin());
				if (id.empty() == false && id.back() == ' ')
					id.pop_back();


				auto it = mLanguageDefinition.mIdentifiers.find(id);
				auto it_l = mLanguageDefinition.mIdentifiers.find(id_lowered);

				if (it != mLanguageDefinition.mIdentifiers.end() || it_l != mLanguageDefinition.mIdentifiers.end()) {
					
					HighlighterSyntax::Instance().DrawTooltip(id);
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
	...
	ChildName = ImGui::GetCurrentWindow()->Name;
...
const TextEditor::Palette & TextEditor::GetDarkPalette()
{
	const static Palette p = { {
			0xff7f7f7f,	// Default
			0xffd69c56,	// Keyword	
			ImGui::ColorConvertFloat4ToU32(ImVec4(0.2f,1.0f,0.2f,1.0f)),	// Number
			0xff7070e0,	// String
			...
			ImGui::ColorConvertFloat4ToU32(ImVec4(0.15f, 0.15f, 0.15f,1.0f)), // Background
			0xffe0e0e0, // Cursor
			0x80a06020, // Selection
			0x800020ff, // ErrorMarker
			ImGui::ColorConvertFloat4ToU32(ImVec4(0.8f, 0.6f, 0.2f,1.0f)), // Background
		...
		for (auto first = bufferBegin; first != last; )
		{
			...
			else
			{
				...
				if (token_color == PaletteIndex::Instruction)
				{
					...
					if (instruction.empty() == false && instruction.front() == ' ')
						instruction.erase(instruction.begin());

					if (instruction.empty() == false && instruction.back() == ' ')
						instruction.pop_back();


					int colorIndex = HighlighterSyntax::Instance().GetTypeFromText(instruction);
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::NASM() {
#ifdef _DEBUG
	std::cout << "const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::NASM()\n";
#endif // _DEBUG
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		...
		for (auto& info : HighlighterSyntax::Instance().GetAllWords()) {
			Identifier id;
			langDef.mIdentifiers.insert(std::make_pair(info, id));
		}

		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_Label(), PaletteIndex::Instruction });
		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_Instruction(), PaletteIndex::Instruction });
		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_AssemblerDirectives(), PaletteIndex::Instruction });

		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_PreprocessorDirectives(), PaletteIndex::Instruction });

		langDef.mTokenRegexStrings.push_back({ "NOI" + HighlighterSyntax::Instance().MakeRegex_PredefinedMacros(), PaletteIndex::Instruction});
		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_Sections(), PaletteIndex::Instruction});

		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_Register_AVX(), PaletteIndex::Instruction });
		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_Register_General(), PaletteIndex::Instruction });
		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_Register_FloatingPoint(), PaletteIndex::Instruction });
		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_Register_Control(), PaletteIndex::Instruction });
		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_Register_Debug(), PaletteIndex::Instruction });
		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_Register_MMX(), PaletteIndex::Instruction });
		langDef.mTokenRegexStrings.push_back({ HighlighterSyntax::Instance().MakeRegex_Register_SSE(), PaletteIndex::Instruction });
