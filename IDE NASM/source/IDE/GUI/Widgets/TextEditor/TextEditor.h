...
public:
	...
	struct LanguageDefinition
	{
		...
		static const LanguageDefinition& NASM();
	};

	TextEditor(FPS_Timer* fps_limiter,const std::wstring& Path);
	~TextEditor();
	...
	void LoadColors(const ObjectColors& object_colors);
...
private:
	...
	struct DataFinder {
		...
		void Prev() {
			...
			index_current--;
			if (index_current < 0)
				index_current = static_cast<int>(FindedWords.size()) - 1;
		...
		void Update(const double& deltatime) {
			for (int i = 0; i < FindedWords.size(); i++){
				FindedWords[i].Attention_Time -= static_cast<float>(deltatime);
				...
				for (int j = 0; j < utf8Line_text[i].size(); j++) {
					...
					if (utf8Line_text[i][j] == utf8_word[0]) {
						...
						if (AllCorrect) {
							...
							pos_word.Find_Start = Coordinates(index_line, len - static_cast<int>(utf8_word.size()));
	...
	void SetCursorAtCoord_ChoosedWord() {
		...
		float lineee = static_cast<float>(_DataFinder.FindedWords[_DataFinder.index_current].Find_Start.mLine - CountLines_In_Window / 2);
